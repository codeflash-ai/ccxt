...
class Exchange(object):
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        if dictionary is not None:
            if key1 in dictionary and dictionary[key1] is not None:
                return str(dictionary[key1])
            if key2 in dictionary and dictionary[key2] is not None:
                return str(dictionary[key2])
        return default_value
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # Fast path for the common case (per profiling): if currency is already a dict with a 'code', skip safe_currency
        if currencyId is None and isinstance(currency, dict) and "code" in currency:
