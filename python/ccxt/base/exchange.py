...
class Exchange(object):
    ...
    def iso8601(timestamp=None):
        if timestamp is None:
            return timestamp
        # Shortcut: skip if not an int or negative
        if type(timestamp) is not int or timestamp < 0:
            return None

        try:
            dt_fromtimestamp = datetime.datetime.fromtimestamp
            dt_timezone_utc = datetime.timezone.utc
            utc = dt_fromtimestamp(timestamp // 1000, dt_timezone_utc)
            dt_strftime = utc.strftime
            ms = int(timestamp) % 1000
            # Only the `%f` needs to be sliced, everything else via locals
            return dt_strftime("%Y-%m-%dT%H:%M:%S.%f")[:-6] + f"{ms:03d}" + "Z"
