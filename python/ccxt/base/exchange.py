...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        # Use setdefault to reduce unnecessary object creation
        ...
        for key in settings:
            ...
            if isinstance(current, dict) and isinstance(val, dict):
                setattr(self, key, self.deep_extend(current, val))
            else:
    ...
    def safe_string(dictionary, key, default_value=None):
        # Use dict.get for a faster check
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Use get pattern for faster lookup then check
        value = dictionary.get(key, None)
        if value is None or value == "":
            return default_value
        try:
    ...
    def safe_number(self, obj, key: IndexType, defaultNumber: Num = None):
        # streamlined deeper method call sequence
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # shortcut the original safe_currency for speed when currency object is provided
        if currencyId is None and currency is not None:
        ...
        if (
            ...
            return self.currencies_by_id[currencyId]["code"]
        code = currencyId
        if currencyId is not None:
            code = self.common_currency_code(currencyId.upper())
        return code
