...
class Exchange(object):
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # Fast path for common cases to reduce function depth and dict creation overhead
        if currencyId is not None and self.currencies_by_id is not None:
            c = self.currencies_by_id.get(currencyId)
            if c is not None:
                return c["code"]
        # fallback, preserves behavioral fidelity
