...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Extremely hot path: avoid repeated key_exists lookup and unnecessary type conversions
        if isinstance(dictionary, dict):
            value = dictionary.get(key, None)
            return str(value) if value is not None and value != "" else default_value
        elif isinstance(dictionary, list):
            if type(key) is int and 0 <= key < len(dictionary):
                value = dictionary[key]
                return (
                    str(value) if value is not None and value != "" else default_value
                )
            return default_value
        elif hasattr(dictionary, "__getitem__") and not isinstance(dictionary, str):
            try:
                value = dictionary[key]
                return (
                    str(value) if value is not None and value != "" else default_value
    ...
    def safe_integer(dictionary, key, default_value=None):
        ...
        if isinstance(dictionary, dict):
            ...
            if not (type(key) is int and 0 <= key < len(dictionary)):
                return default_value
            value = dictionary[key]
            if value is None or value == "":
                return default_value
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        val1 = Exchange.safe_string(dictionary, key1)
        return (
            val1
            if val1 is not None
    ...
    def iso8601(timestamp=None):
        if timestamp is None:
            return timestamp
        if not isinstance(timestamp, int) or int(timestamp) < 0:
            return None
        ...
        try:
            ...
            # Precompute formatted string, avoid multiple slicing and formatting operations
            milli = int(timestamp) % 1000
            dt_str = utc.strftime("%Y-%m-%dT%H:%M:%S")
            return f"{dt_str}.{milli:03d}Z"
        except Exception:
            return None
    ...
    def safe_dict_n(self, dictionary, keys, defaultValue=None):
        for key in keys:
            value = None
            if isinstance(dictionary, dict):
                value = dictionary.get(key, None)
    ...
    def safe_bool_n(self, dictionary, keys, defaultValue=None):
        for key in keys:
            value = None
            if isinstance(dictionary, dict):
                value = dictionary.get(key, None)
