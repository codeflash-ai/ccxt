...
class Exchange(object):
    ...
    def keysort(dictionary):
        # Return a sorted list of tuples; avoids OrderedDict construction unless strictly needed
        return sorted(dictionary.items(), key=lambda t: t[0])
    ...
    def implode_params(string, params):
        if isinstance(params, dict):
            ...
            # Only replace for keys found in {} braces
            string = re.sub(r"{([\w-]+)}", repl, string)
        return string

    @staticmethod
    def urlencode(params={}, doseq=False, sort=False):
        # Only convert boolean values if present, avoid dict copy if possible
        if not params:
        ...
        for v in params.values():
            if isinstance(v, bool):
                ...
                break
        if has_bool:
            # Only mutate keys in a copy that are boolean.
            newParams = {
    ...
    def omit(d, *args):
        if isinstance(d, dict):
            ...
            for arg in args:
                if type(arg) is list:
                    keys_to_remove.update(arg)
                else:
                    keys_to_remove.add(arg)
