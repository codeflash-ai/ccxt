...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        for key, value in settings.items():
            ...
            if isinstance(current, dict) and isinstance(value, dict):
                # For shallow config merge, use dict unpacking for speed
        ...
        # Avoid repeated setattr/lookups for each attribute
        for name in names:
            parts = name.split("_")
        ...
        # Session setup
        if not getattr(self, "session", None) and getattr(self, "synchronous", True):
            s = Session()
            s.trust_env = self.requests_trust_env
            self.session = s
        # Setup logger if not already
        self.logger = getattr(self, "logger", None) or logging.getLogger(__name__)
    ...
    def safe_string(dictionary, key, default_value=None):
        # Direct dictionary key check for speed, assumes typical dict usage
        return (
            str(dictionary[key])
