...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Inline key_exists for dict for speed in hot path
    ...
    def safe_balance(self, balance: dict):
        # Omit certain keys efficiently
        to_omit = {"info", "timestamp", "datetime", "free", "used", "total"}
        balances = {k: v for k, v in balance.items() if k not in to_omit}
        # Avoid list allocation then indexing: use direct dict iteration
        balance_free = {}
        balance_used = {}
        balance_total = {}
        debtBalance = {}
        for code, entry in balances.items():
            total = self.safe_string(entry, "total")
            free = self.safe_string(entry, "free")
            used = self.safe_string(entry, "used")
            debt = self.safe_string(entry, "debt")
            # Reduce logic calls by caching values
            ...
            entry["free"] = self.parse_number(free)
            entry["used"] = self.parse_number(used)
            entry["total"] = self.parse_number(total)
            balance_free[code] = entry["free"]
            balance_used[code] = entry["used"]
            balance_total[code] = entry["total"]
            if debt is not None:
                entry["debt"] = self.parse_number(debt)
