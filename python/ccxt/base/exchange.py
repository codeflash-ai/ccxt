...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Fast-path for flat dicts, avoids Exchange.key_exists overhead
        try:
            value = dictionary[key]
        except (KeyError, TypeError):
            return default_value
        # branch for None without creating a string object if unnecessary
    ...
    def iso8601(timestamp=None):
        ...
        try:
            ...
            # Avoid slicing; use f-string for improved efficiency
            base = utc.strftime("%Y-%m-%dT%H:%M:%S")
            return f"{base}.{str(ms).zfill(3)}Z"
    ...
    def safe_balance(self, balance: dict):
        # Use dict comprehension for improved performance and less function call overhead
        skip = {"info", "timestamp", "datetime", "free", "used", "total"}
        # Build balances dict directly, omit keys
        balances = {k: v for k, v in balance.items() if k not in skip}
        codes = list(balances.keys())
        free_dict = {}
        used_dict = {}
        total_dict = {}
        debtBalance = {}
        parse_number = self.parse_number  # method binding optimization

        sbal = self.safe_string
        Precise_add = Precise.string_add
        Precise_sub = Precise.string_sub

        for code in codes:
            entry = balance[code]
            total = sbal(entry, "total")
            free = sbal(entry, "free")
            used = sbal(entry, "used")
            debt = sbal(entry, "debt")

            if (total is None) and (free is not None) and (used is not None):
                total = Precise_add(free, used)
            if (free is None) and (total is not None) and (used is not None):
                free = Precise_sub(total, used)
            if (used is None) and (total is not None) and (free is not None):
                used = Precise_sub(total, free)

            entry["free"] = parse_number(free)
            entry["used"] = parse_number(used)
            entry["total"] = parse_number(total)
            free_dict[code] = entry["free"]
            used_dict[code] = entry["used"]
            total_dict[code] = entry["total"]
            if debt is not None:
                entry["debt"] = parse_number(debt)
                debtBalance[code] = entry["debt"]
        balance["free"] = free_dict
        balance["used"] = used_dict
