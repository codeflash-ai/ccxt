...
class Exchange(object):
    ...
    def safe_currency(self, currencyId: Str, currency: Currency = None):
        if (currencyId is None) and (currency is not None):
            return currency
        # Fast-path dictionary lookup and minimize assignments
        currencies_by_id = self.currencies_by_id
        if (
            (currencies_by_id is not None)
            and (currencyId in currencies_by_id)
            and (currencies_by_id[currencyId] is not None)
        ):
            return currencies_by_id[currencyId]
        code = currencyId
