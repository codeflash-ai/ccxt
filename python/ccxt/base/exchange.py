...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Inline Exchange.key_exists for speed
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Inline key search and reduce try/except scope
        value = None
        if isinstance(dictionary, dict):
            value = dictionary.get(key)
        elif (
    ...
    def iso8601(timestamp=None):
        ...
        try:
            utc = datetime.datetime.fromtimestamp(
                timestamp_int // 1000, datetime.timezone.utc
            )
            ms = timestamp_int % 1000
            # Preallocate the formatted string
            s = utc.strftime("%Y-%m-%dT%H:%M:%S.{:03d}Z").format(ms)
            return s
    ...
    def safe_dict(self, dictionary, key: IndexType, defaultValue: dict = None):
        ...
        @ignore
               ...
               :returns dict | None:
        """
        # Directly check and return, avoid method indirection
        value = None
        if isinstance(dictionary, dict):
            value = dictionary.get(key, defaultValue)
        elif (
            isinstance(dictionary, list)
            and isinstance(key, int)
            and 0 <= key < len(dictionary)
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # Also uses above safe_string optimization
        code_currency = self.safe_currency(currencyId, currency)
        return code_currency["code"]
