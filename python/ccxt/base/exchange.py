...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Highly performance-sensitive: avoid double lookup & unnecessary checks
        # The line profile shows that all uses are with dicts with the right key types.
        try:
            value = dictionary[key]
    ...
    def safe_balance(self, balance: dict):
        ...
        string_add = Precise.string_add
        string_sub = Precise.string_sub

        # Omit meta keys, get codes
        balances = omit(
            balance, ["info", "timestamp", "datetime", "free", "used", "total"]
        )
        codes = list(balances.keys())
        # Preallocate output dicts
        balance_free = {}
        balance_used = {}
        balance_total = {}
        debtBalance = {}

        balance["free"] = balance_free
        balance["used"] = balance_used
        balance["total"] = balance_total

        for code in codes:
            bcode = balance[code]
            total = safe_string(bcode, "total")
            free = safe_string(bcode, "free")
            used = safe_string(bcode, "used")
            debt = safe_string(bcode, "debt")

            if total is None and free is not None and used is not None:
                total = string_add(free, used)
            if free is None and total is not None and used is not None:
                free = string_sub(total, used)
            if used is None and total is not None and free is not None:
                used = string_sub(total, free)

            parsed_free = parse_number(free)
            parsed_used = parse_number(used)
            parsed_total = parse_number(total)

            bcode["free"] = parsed_free
            bcode["used"] = parsed_used
            bcode["total"] = parsed_total

            balance_free[code] = parsed_free
            balance_used[code] = parsed_used
            balance_total[code] = parsed_total
            if debt is not None:
                bcode["debt"] = parse_number(debt)
                debtBalance[code] = bcode["debt"]
        if debtBalance:
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        currency_val = self.safe_currency(currencyId, currency)
        return currency_val["code"]
