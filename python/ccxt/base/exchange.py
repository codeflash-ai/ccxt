...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Fast-path dict and list key existence check, optimize away repeated function calls
        if isinstance(dictionary, dict):
            ...
            # Only allow integer indices for lists (per original key_exists logic)
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Fast-path dict/list key existence check
        if isinstance(dictionary, dict):
            if (
                key in dictionary
                and dictionary[key] is not None
