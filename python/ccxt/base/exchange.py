...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # HOT PATH; avoid custom key_exists machinery for fast dict lookup
        # Only use slow fallback if needed
        try:
            ...
            # Fall back to original behavior (list/str/unknown types)
            return (
                str(dictionary[key])
                if Exchange.key_exists(dictionary, key)
                else default_value
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Fast path: dict, else fallback to original family
        if isinstance(dictionary, dict):
            v = dictionary.get(key1, None)
            if v is not None and v != "":
                return str(v)
            v = dictionary.get(key2, None)
            if v is not None and v != "":
                return str(v)
            return default_value
        return Exchange.safe_either(
    ...
    def safe_string_n(dictionary, key_list, default_value=None):
        # HOT PATH: optimize for dict lookups
        if isinstance(dictionary, dict):
            for key in key_list:
                v = dictionary.get(key, None)
                if v is not None and v != "":
    ...
    def safe_dict(self, dictionary, key: IndexType, defaultValue: dict = None):
        ...
        @ignore
               ...
               :returns dict | None:
        """
        # HOT PATH: optimize fast dict lookup
        if isinstance(dictionary, dict):
            v = dictionary.get(key, None)
    ...
    def safe_list(
        ...
        @ignore
               ...
               :returns Array | None:
        """
        # HOT PATH: optimize for list-in-dict
        if isinstance(dictionaryOrList, dict):
            v = dictionaryOrList.get(key, None)
    ...
    def safe_balance(self, balance: dict):
        ...
        for code in codes:
            bc = balance[code]
            # Inline safe_string (bottleneck): we know bc is a dict
            total_v = bc.get("total", None)
            if total_v is None or total_v == "":
                total_val = None
            else:
                total_val = str(total_v)
            free_v = bc.get("free", None)
            if free_v is None or free_v == "":
                free_val = None
            else:
                free_val = str(free_v)
            used_v = bc.get("used", None)
            if used_v is None or used_v == "":
                used_val = None
            else:
                used_val = str(used_v)
            debt_v = bc.get("debt", None)
            if debt_v is None or debt_v == "":
                debt_val = None
            else:
                debt_val = str(debt_v)
            ...
            if (
                (total_val is None)
                and (free_val is not None)
                and (used_val is not None)
            ):
                total_val = Precise.string_add(free_val, used_val)
            if (
                (free_val is None)
                and (total_val is not None)
                and (used_val is not None)
            ):
                free_val = Precise.string_sub(total_val, used_val)
            if (
                (used_val is None)
                and (total_val is not None)
                and (free_val is not None)
            ):
                used_val = Precise.string_sub(total_val, free_val)
            bc["free"] = self.parse_number(free_val)
            bc["used"] = self.parse_number(used_val)
            bc["total"] = self.parse_number(total_val)
            free[code] = bc["free"]
            used[code] = bc["used"]
            total[code] = bc["total"]
            if debt_val is not None:
                bc["debt"] = self.parse_number(debt_val)
                debtBalance[code] = bc["debt"]
        balance["free"] = free
        balance["used"] = used
        balance["total"] = total
        if debtBalance:
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # Hot path: avoid function call overhead for extremely common not-None case
        if currencyId is not None or currency is not None:
