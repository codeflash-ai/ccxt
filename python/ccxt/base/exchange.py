...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        # Fast property initialization: use dict literals directly
    ...
    def safe_string(dictionary, key, default_value=None):
        # Fast-path for dict or list key lookup, avoid Exchange.key_exists overhead
        # Covers list (index) and dict (key)
        # Fails for str, acts exactly as before behaviorally
        if hasattr(dictionary, "__getitem__") and not isinstance(dictionary, str):
            try:
                val = dictionary[key]
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Fast-path for dict/list, avoid Exchange.key_exists overhead
        if hasattr(dictionary, "__getitem__") and not isinstance(dictionary, str):
            try:
                val = dictionary[key]
                if val is None or val == "":
                    return default_value
                # int(float(...)) to handle '100.0' strings
    ...
    def iso8601(timestamp=None):
        ...
        try:
            ...
            utc = datetime.datetime.utcfromtimestamp(sec)
            # Only slice NECESSARY parts, format ms optimally
            return utc.strftime("%Y-%m-%dT%H:%M:%S.") + "{:03d}Z".format(ms)
    ...
    def safe_number(self, obj, key: IndexType, defaultNumber: Num = None):
        ...
        if hasattr(obj, "__getitem__") and not isinstance(obj, str):
            try:
                ...
                pass
        if val is None:
            val = self.safe_string(obj, key)  # fallback, preserves edge-case behavior
        return self.parse_number(val, defaultNumber)
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # If currencyId is a string code and currencies_by_id is not set, fast-path-out
        # Otherwise keep legacy full logic
        return self.safe_currency(currencyId, currency)["code"]
    ...
    def safe_symbol(
        ...
        return self.safe_market(marketId, market, delimiter, marketType)["symbol"]
