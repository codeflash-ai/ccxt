...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Optimized version: avoid Exchange.key_exists logic, instead use dict.get and explicit None/empty value checks.
        if not isinstance(dictionary, str) and hasattr(
            dictionary, "__getitem__"
        ):  # skip string keys, require dict or list
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Inline short equivalent of Exchange.safe_either(Exchange.safe_string, ...)
        v = Exchange.safe_string(dictionary, key1)
        return (
            v
            if v is not None
            else Exchange.safe_string(dictionary, key2, default_value)
        )
    ...
    def safe_order(self, order: dict, market: Market = None):
        ...
        amount = self.omit_zero(safe_string(order, "amount"))
        remaining = safe_string(order, "remaining")
        filled = safe_string(order, "filled")
        cost = safe_string(order, "cost")
        average = self.omit_zero(safe_string(order, "average"))
        price = self.omit_zero(safe_string(order, "price"))
        ...
        if shouldParseFees:
            ...
            for i in range(0, reducedLength):
                reducedFees[i]["cost"] = safe_number(reducedFees[i], "cost")
                if "rate" in reducedFees[i]:
                    reducedFees[i]["rate"] = safe_number(reducedFees[i], "rate")
        ...
        contractSize = self.number_to_string(safe_value(market, "contractSize", 1))
        # inverse
        ...
        for i in range(0, len(trades)):
            ...
            for j in range(0, len(entryFees)):
                entryFees[j]["cost"] = safe_number(entryFees[j], "cost")
        ...
        datetime_val = safe_string(order, "datetime")
        if datetime_val is None:
            datetime_val = self.iso8601(timestamp)
        ...
        # All dict lookups cached, property access optimized.
        return self.extend(
            order,
            {
