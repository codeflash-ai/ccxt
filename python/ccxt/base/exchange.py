...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Highly optimized for python dict/key-pattern
        try:
            val = dictionary[key]
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Inlined for perf, avoids unnecessary function call stack
        try:
    ...
    def safe_integer_2(dictionary, key1, key2, default_value=None):
        # Inlined for perf, avoids call to safe_either
        def safe_integer(dictionary, key, default_value=None):
            try:
                val = dictionary[key]
        ...
        v1 = safe_integer(dictionary, key1)
        if v1 is not None:
            return v1
        return safe_integer(dictionary, key2, default_value)
    ...
    def iso8601(timestamp=None):
        ...
        try:
            ...
            utc = datetime.datetime.utcfromtimestamp(base_seconds)
            # Precompute format then append ms for perf
            return (
                utc.strftime("%Y-%m-%dT%H:%M:%S.") + "{:03d}".format(milliseconds) + "Z"
            )
        except (
            TypeError,
            OverflowError,
            OSError,
            ValueError,
        ):  # ValueError for some non-int types
            return None
    ...
    def safe_trade(self, trade: dict, market: Market = None):
        ...
        cost = safe_string(trade, "cost")
        amount = safe_string(trade, "amount")
        price = safe_string(trade, "price")
        if cost is None:
            contractSize = safe_string(market, "contractSize") if market else None
            multiplyPrice = price
        ...
        trade["amount"] = parse_number(amount)
        trade["price"] = parse_number(price)
        trade["cost"] = parse_number(cost)
        return trade
