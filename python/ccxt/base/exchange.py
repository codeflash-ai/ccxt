...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Optimized: fast path for dict & list, avoid key_exists overhead and redundant checks
        if isinstance(dictionary, dict):
            ...
            # For lists, fast path for int key
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Optimized: Avoid key_exists by using dict/list logic
        if isinstance(dictionary, dict):
            val = dictionary.get(key, None)
        elif (
            isinstance(dictionary, list)
            and isinstance(key, int)
            and 0 <= key < len(dictionary)
        ):
            val = dictionary[key]
        else:
            return default_value
    ...
    def safe_value(dictionary, key, default_value=None):
        # Optimized: avoid double lookup for dict and list
        if isinstance(dictionary, dict):
            val = dictionary.get(key, None)
            return val if val is not None and val != "" else default_value
        elif (
            isinstance(dictionary, list)
            and isinstance(key, int)
            and 0 <= key < len(dictionary)
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Inline safe_either logic for hot path
        v1 = Exchange.safe_string(dictionary, key1)
        return (
            v1
            if v1 is not None
    ...
    def safe_string_upper_2(dictionary, key1, key2, default_value=None):
        v1 = Exchange.safe_string_upper(dictionary, key1)
        return (
            v1
            if v1 is not None
            else Exchange.safe_string_upper(dictionary, key2, default_value)
        )
    ...
    def safe_timestamp_2(dictionary, key1, key2, default_value=None):
        v1 = Exchange.safe_integer(dictionary, key1)
        if v1 is not None:
            return v1 * 1000
        v2 = Exchange.safe_integer(dictionary, key2, default_value)
        return v2 * 1000 if v2 is not None else default_value
    ...
    def iso8601(timestamp=None):
        ...
        try:
            ...
            # Using f-string formatting for better performance
    ...
    def safe_market(
        ...
        if marketId is not None:
            if (self.markets_by_id is not None) and (marketId in self.markets_by_id):
                ...
                if len(parts) == 2:
                    ...
                    symbol = base + "/" + quote
                    # Only build exactly what's needed
                    return {
                        "symbol": symbol,
                        "baseId": baseId,
                        "quoteId": quoteId,
        ...
        if market is not None:
            return market
        # Instead of re-calling safe_market_structure, build a lean result
    ...
    def safe_symbol(
        ...
        market_obj = self.safe_market(marketId, market, delimiter, marketType)
        return market_obj["symbol"]
