...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Fast path: dictionary is a dict, not a str, and key exists
        if (
            isinstance(dictionary, dict)
            and key in dictionary
            and dictionary[key] not in (None, "")
        ):
    ...
    def safe_dict(self, dictionary, key: IndexType, defaultValue: dict = None):
        ...
        @ignore
               ...
               :returns dict | None:
        """
        # Fast path for dict extraction, avoids nested calls when type is guaranteed
        value = dictionary.get(key) if isinstance(dictionary, dict) else None
    ...
    def network_code_to_id(self, networkCode: str, currencyCode: Str = None):
        ...
        networkIdsByCodes = options.get("networks", {})
        networkId = Exchange.safe_string(networkIdsByCodes, networkCode)
        # for example, if 'ETH' is passed for networkCode, but 'ETH' key not defined in `options->networks` object
        if networkId is None:
            if currencyCode is None:
                # Search in currencies for a network id match
                for currency in self.currencies.values():
                    networks = self.safe_dict(currency, "networks")
                    network = self.safe_dict(networks, networkCode)
                    nid = Exchange.safe_string(network, "id")
                    if nid is not None:
                        networkId = nid
                        break
            else:
                defaultNetworkCodeReplacements = options.get(
                    "defaultNetworkCodeReplacements", {}
                )
                if currencyCode in defaultNetworkCodeReplacements:
                    ...
                    for key, value in replacementObject.items():
                        # if value matches to provided unified networkCode, then we use it's key to find network-id in `options->networks` object
                        if value == networkCode:
                            networkId = Exchange.safe_string(networkIdsByCodes, key)
                            break
                else:
                    ...
                    networks = self.safe_dict(currency, "networks")
                    network = self.safe_dict(networks, networkCode)
                    networkId = Exchange.safe_string(network, "id")
    ...
    def default_network_code(self, currencyCode: str):
        ...
        if currencyCode in defaultNetworks:
            return defaultNetworks[currencyCode]
        # Try global-scope fallback without extra safe_string call
        defaultNetwork = self.options.get("defaultNetwork", None)
    ...
    def currency(self, code: str):
        # No need to build list(self.currencies.keys()) and count for membership
        if not self.currencies:
            raise ExchangeError(str(self.id) + " currencies not loaded")
        if isinstance(code, str):
            if code in self.currencies:
                return self.currencies[code]
            elif hasattr(self, "currencies_by_id") and code in getattr(
                self, "currencies_by_id"
            ):
                return getattr(self, "currencies_by_id")[code]
        raise ExchangeError(str(self.id) + " does not have currency code " + code)
