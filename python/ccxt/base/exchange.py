...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        ...
        if isinstance(dictionary, dict):
            ...
            try:
                ...
                pass
            return default_value
        # Fallback to original key_exists
        return (
            str(dictionary[key])
            if Exchange.key_exists(dictionary, key)
            else default_value
    ...
    def safe_balance(self, balance: dict):
        ...
        for code in codes:
            account_dict = balance[code]
            total = safe_string(account_dict, "total")
            free = safe_string(account_dict, "free")
            used = safe_string(account_dict, "used")
            debt = safe_string(account_dict, "debt")
            ...
            val_free = parse_number(free)
            val_used = parse_number(used)
            val_total = parse_number(total)
            account_dict["free"] = val_free
            account_dict["used"] = val_used
            account_dict["total"] = val_total
            balance_free[code] = val_free
            balance_used[code] = val_used
            balance_total[code] = val_total
            if debt is not None:
                debt_val = parse_number(debt)
                account_dict["debt"] = debt_val
                debtBalance[code] = debt_val
        if debtBalance:
