...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        # Use dict() only if attribute is None, else assign existing object
        self.precision = self.precision if self.precision is not None else {}
        self.limits = self.limits if self.limits is not None else {}
        self.exceptions = self.exceptions if self.exceptions is not None else {}
        self.headers = self.headers if self.headers is not None else {}
        self.balance = self.balance if self.balance is not None else {}
        self.orderbooks = self.orderbooks if self.orderbooks is not None else {}
        self.fundingRates = self.fundingRates if self.fundingRates is not None else {}
        self.tickers = self.tickers if self.tickers is not None else {}
        self.bidsasks = self.bidsasks if self.bidsasks is not None else {}
        self.trades = self.trades if self.trades is not None else {}
        self.transactions = self.transactions if self.transactions is not None else {}
        self.ohlcvs = self.ohlcvs if self.ohlcvs is not None else {}
        self.liquidations = self.liquidations if self.liquidations is not None else {}
        self.myLiquidations = (
            self.myLiquidations if self.myLiquidations is not None else {}
        )
        self.currencies = self.currencies if self.currencies is not None else {}
        self.options = (
            self.options if self.options is not None else self.get_default_options()
        )
    ...
    def iso8601(timestamp=None):
        ...
        try:
            base_seconds = timestamp // 1000
            ms = int(timestamp) % 1000
            dt = datetime.datetime.utcfromtimestamp(base_seconds)
            # Compose with efficient string formatting
            return dt.strftime("%Y-%m-%dT%H:%M:%S.") + "{:03d}Z".format(ms)
