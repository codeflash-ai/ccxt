...
_PARSE8601_TZ_REGEX = re.compile(
    r"([0-9]{4})-?"  # yyyy
    r"([0-9]{2})-?"  # mm
    r"([0-9]{2})(?:T|[\s])?"  # dd
    r"([0-9]{2}):?"  # h
...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Optimized: inline key lookup and str conversion, avoid function call overhead
        if (
    ...
    def safe_string_upper(dictionary, key, default_value=None):
        # Optimized: direct lookup, single call, local var reuse. Fast default return.
        if (
            isinstance(dictionary, dict)
            and key in dictionary
            and dictionary[key] is not None
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Optimized: avoid function indirection, explicit fast key checks for both possible keys
        if isinstance(dictionary, dict):
            value1 = dictionary.get(key1, None)
            if value1 is not None and value1 != "":
                return str(value1)
            value2 = dictionary.get(key2, None)
    ...
    def iso8601(timestamp=None):
        ...
        try:
            ...
            # Avoid slicing strftime result: direct efficient append
            return f"{s}.{msec:03d}Z"
    ...
    def parse8601(timestamp=None):
        ...
        match = _PARSE8601_TZ_REGEX.search(timestamp)
        if match is None:
            return None
        ...
        hours_offset = int(hours or 0) * sign_val
        minutes_offset = int(minutes or 0) * sign_val
