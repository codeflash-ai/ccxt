...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Fast path: try direct access first, avoid hasattr/isinstance in key_exists if dict
        if isinstance(dictionary, dict):
            try:
                value = dictionary[key]
                return (
                    str(value) if value is not None and value != "" else default_value
                )
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        value1 = Exchange.safe_string(dictionary, key1)
    ...
    def safe_integer_n(dictionary, key_list, default_value=None):
        # optimize for dict and inline conversion
        if isinstance(dictionary, dict):
            for key in key_list:
                value = dictionary.get(key)
                if value is not None and value != "":
    ...
    def iso8601(timestamp=None):
        ...
        try:
            utc = datetime.datetime.fromtimestamp(ts_int // 1000, datetime.timezone.utc)
            # Faster strftime + string concat
            ms = ts_int % 1000
    ...
    def safe_bool(self, dictionary, key: IndexType, defaultValue: bool = None):
        ...
        @ignore
               ...
               :returns bool | None:
        """
        # optimize: try fast key lookup for dict
        if isinstance(dictionary, dict):
            value = dictionary.get(key)
            if isinstance(value, bool):
                return value
    ...
    def safe_trade(self, trade: dict, market: Market = None):
        ...
        inverse = False
        if cost is None:
            contractSize = self.safe_string(market, "contractSize") if market else None
            multiplyPrice = price
        ...
        if marketId is not None:
            if (markets_by_id is not None) and (marketId in markets_by_id):
                ...
                if numMarkets == 1:
                    ...
                    for curMarket in markets:
                        if curMarket.get(marketType):
                            return curMarket
            elif delimiter:
                ...
                if len(parts) == 2:
                    ...
                    # Extract currency codes only once for base/quote
    ...
    def safe_currency_code(self, currencyId: Str, currency: Currency = None):
        # Inline method for speed; reduce repeated method call
        currency_obj = self.safe_currency(currencyId, currency)
        return currency_obj["code"]
