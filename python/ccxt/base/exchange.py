...
class Exchange(object):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Use dict.get for O(1) and less code-path branching
        if isinstance(dictionary, dict):
            val = dictionary.get(key, default_value)
            return str(val) if val is not None else default_value
    ...
    def safe_integer(dictionary, key, default_value=None):
        # (dict.get saves lookup cost, avoids unnecessary fallback)
        if isinstance(dictionary, dict):
            val = dictionary.get(key)
            if val is None:
                return default_value
            try:
                # skip float() if value is int already
    ...
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Directly try first key then second; no nested calls
        if isinstance(dictionary, dict):
            ...
            return default_value
        return Exchange.safe_either(
            Exchange.safe_string, dictionary, key1, key2, default_value
        )
    ...
    def iso8601(timestamp=None):
        ...
        try:
            dt = datetime.datetime.fromtimestamp(ts // 1000, UTC)
            # Compose string only once (avoid repeated slicing)
            ms = ts % 1000
            s = dt.strftime(STRFTIME_FORMAT)
            return s[:-6] + f"{ms:03d}Z"
    ...
    def safe_order(self, order: dict, market: Market = None):
        # Bulk local refs to methods for perf
        ...
        if parseFilled or parseCost or shouldParseFees:
            ...
            if isinstance(trades, list) and trades:
                t0 = trades[0]
                # Promote fields up from first trade when missing
                for merge_key, order_key in (
                    ("symbol", "symbol"),
                    ("side", "side"),
                    ("type", "type"),
                    ("order", "id"),
                ):
                    if order.get(order_key) is None and t0.get(merge_key) is not None:
                        order[order_key] = t0[merge_key]
                if parseFilled:
                    filled = "0"
                if parseCost:
                    cost = "0"
                for trade in trades:
                    ...
                    if parseSymbol and trade.get("symbol") is not None:
                        symbol = safe_string(trade, "symbol")
                    if parseSide and trade.get("side") is not None:
                    ...
                    if parseLastTradeTimeTimestamp and tradeTimestamp is not None:
                        lastTradeTimeTimestamp = (
                            ...
                            else max(lastTradeTimeTimestamp, tradeTimestamp)
                        )
                    if shouldParseFees:
                        tradeFees = safe_value(trade, "fees")
                        if tradeFees:
                            fees.extend([extend({}, tf) for tf in tradeFees])
                        else:
        ...
        if shouldParseFees:
            ...
            for rf in reducedFees:
                rf["cost"] = safe_number(rf, "cost")
                if "rate" in rf:
                    rf["rate"] = safe_number(rf, "rate")
            if not parseFee and (reducedLength == 0):
                feeCopy = extend(fee)
                feeCopy["cost"] = safe_number(feeCopy, "cost")
                if "rate" in feeCopy:
                    feeCopy["rate"] = safe_number(feeCopy, "rate")
        ...
        if average is None:
            if (
                ...
                and Precise.string_gt(filled, "0")
            ):
        ...
        if parseCost and (filled is not None) and costPriceExists:
            multiplyPrice = price if average is None else average
            # contract trading
            filledTimesContractSize = Precise.string_mul(filled, contractSize)
            if inverse:
        ...
        if emptyPrice and (orderType == "market"):
            price = average
        for entry in trades:
            entry["amount"] = safe_number(entry, "amount")
            entry["price"] = safe_number(entry, "price")
            entry["cost"] = safe_number(entry, "cost")
            tradeFee = self.safe_dict(entry, "fee", {})
            tradeFee["cost"] = safe_number(tradeFee, "cost")
            if "rate" in tradeFee:
                tradeFee["rate"] = safe_number(tradeFee, "rate")
            entryFees = safe_list(entry, "fees", [])
            for entryFee in entryFees:
                entryFee["cost"] = safe_number(entryFee, "cost")
        ...
        triggerPrice = self.parse_number(
            safe_string_2(order, "triggerPrice", "stopPrice")
        )
        takeProfitPrice = self.parse_number(safe_string(order, "takeProfitPrice"))
        stopLossPrice = self.parse_number(safe_string(order, "stopLossPrice"))
        return extend(
            order,
            {
    ...
    def safe_market(
        ...
        # use local refs for methods that are used multiple times
        safe_market_structure = self.safe_market_structure
        safe_string = self.safe_string
        safe_currency_code = self.safe_currency_code
        safe_value = self.safe_value
        result = safe_market_structure(
            {
                "symbol": marketId,
                "marketId": marketId,
            }
        ...
        if marketId is not None:
            ...
            if (markets_by_id is not None) and (marketId in markets_by_id):
                markets = markets_by_id[marketId]
                ...
                if numMarkets == 1:
                    ...
                    if marketType is None:
                        if market is None:
                            raise ArgumentsRequired(
                                f"{self.id} safeMarket() requires a fourth argument for {marketId} to disambiguate between different markets with the same market id"
                            )
                        else:
                            marketType = market["type"]
                    for m in markets:
                ...
                if len(parts) == 2:
                    baseId, quoteId = parts[0], parts[1]
                    base = safe_currency_code(baseId)
                    quote = safe_currency_code(quoteId)
                    symbol = base + "/" + quote
