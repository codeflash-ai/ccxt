...
from ccxt.static_dependencies.starknet.hash.utils import (
    message_signature,
...
class Exchange(object):
    ...
    options = (
        None
    )  # Python does not allow to define properties in run-time with setattr
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        self.myLiquidations = (
            dict() if self.myLiquidations is None else self.myLiquidations
        )
        self.currencies = dict() if self.currencies is None else self.currencies
        self.options = (
            self.get_default_options() if self.options is None else self.options
        )  # Python does not allow to define properties in run-time with setattr
    ...
    def __repr__(self):
        return (
            "ccxt." + ("async_support." if self.asyncio_loop else "") + self.id + "()"
        )
    ...
    def fetch(self, url, method="GET", headers=None, body=None):
        ...
        httpProxy, httpsProxy, socksProxy = self.check_proxy_settings(
        ...
        if self.proxies is not None:
            if proxyAgentSet or proxyUrl:
                raise ExchangeError(
                    self.id
                    + " you have conflicting proxy settings - use either .proxies or http(s)Proxy / socksProxy / proxyUrl"
                )
        ...
        try:
            ...
            if (
                json_response
                and not isinstance(json_response, list)
                and self.returnResponseHeaders
            ):
    ...
    def safe_string(dictionary, key, default_value=None):
        # Inlined (fast) version of Exchange.key_exists for performance
        # In the real code, Exchange.key_exists is:
        #  - hasattr(dictionary, '__getitem__') and not isinstance(dictionary, str)
        #  - if list, key must be int
        #  - try: value = dictionary[key]; return not None and not ''
        #  - except LookupError: return False
        # This inline handles all cases, but substantially reduces function calls.
        if isinstance(dictionary, dict):
            v = dictionary.get(key, None)
            return str(v) if v is not None and v != "" else default_value
    ...
    def safe_value(dictionary, key, default_value=None):
        return (
            dictionary[key] if Exchange.key_exists(dictionary, key) else default_value
        )
    ...
    def safe_float_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_either(
            Exchange.safe_float, dictionary, key1, key2, default_value
        )

    @staticmethod
    def safe_string_2(dictionary, key1, key2, default_value=None):
        # Inline and avoid safe_either indirection
        v = Exchange.safe_string(dictionary, key1)
        if v is not None:
            return v
        return Exchange.safe_string(dictionary, key2, default_value)

    @staticmethod
    def safe_string_lower_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_either(
            Exchange.safe_string_lower, dictionary, key1, key2, default_value
        )

    @staticmethod
    def safe_string_upper_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_either(
            Exchange.safe_string_upper, dictionary, key1, key2, default_value
        )

    @staticmethod
    def safe_integer_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_either(
            Exchange.safe_integer, dictionary, key1, key2, default_value
        )
    ...
    def safe_integer_product_2(dictionary, key1, key2, factor, default_value=None):
        ...
        return (
            ...
            else Exchange.safe_integer_product(dictionary, key2, factor, default_value)
        )

    @staticmethod
    def safe_timestamp_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_integer_product_2(
            dictionary, key1, key2, 1000, default_value
        )

    @staticmethod
    def safe_value_2(dictionary, key1, key2, default_value=None):
        return Exchange.safe_either(
            Exchange.safe_value, dictionary, key1, key2, default_value
        )
    ...
    def safe_timestamp_n(dictionary, key_list, default_value=None):
        return Exchange.safe_integer_product_n(
            dictionary, key_list, 1000, default_value
        )
    ...
    def get_object_value_from_key_list(dictionary_or_list, key_list):
        ...
        for key in key_list:
            if isDataDict:
                if (
                    key in dictionary_or_list
                    and dictionary_or_list[key] is not None
                    and dictionary_or_list[key] != ""
                ):
                    return dictionary_or_list[key]
            elif isDataArray and not isinstance(key, str):
                if (
                    (key < len(dictionary_or_list))
                    and (dictionary_or_list[key] is not None)
                    and (dictionary_or_list[key] != "")
                ):
                    return dictionary_or_list[key]
    ...
    def group_by(array, key):
        ...
        array = Exchange.to_array(array)
        array = [
            entry for entry in array if (key in entry) and (entry[key] is not None)
        ]
        for entry in array:
    ...
    def index_by(array, key):
        ...
        for element in array:
            if ((is_int_key and (key < len(element))) or (key in element)) and (
    ...
    def sort_by_2(array, key1, key2, descending=False):
        return sorted(
            array,
            key=lambda k: (
                k[key1] if k[key1] is not None else "",
                k[key2] if k[key2] is not None else "",
            ),
            reverse=descending,
        )
    ...
    def dmy(timestamp, infix="-"):
        utc_datetime = datetime.datetime.fromtimestamp(
            int(round(timestamp / 1000)), datetime.timezone.utc
        )
    ...
    def parse_date(timestamp=None):
        ...
        if "GMT" in timestamp:
            try:
                string = (
                    "".join([str(value).zfill(2) for value in parsedate(timestamp)[:6]])
                    + ".000Z"
                )
    ...
    def jwt(request, secret, algorithm="sha256", is_rsa=False, opts={}):
        ...
        encoded_data = Exchange.urlencode_base64(
            Exchange.encode(Exchange.json(request))
        )
        ...
        if is_rsa or algoType == "RS":
            ...
            signature = Exchange.hmac(
                Exchange.encode(token), secret, algos[algorithm], "binary"
            )
    ...
    def rsa(request, secret, alg="sha256"):
        ...
        priv_key = load_pem_private_key(
            Exchange.encode(secret), None, backends.default_backend()
        )
        return Exchange.binary_to_base64(
            priv_key.sign(Exchange.encode(request), padding.PKCS1v15(), algorithm)
        )
    ...
    def ecdsa(request, secret, algorithm="p256", hash=None, fixed_length=False):
        ...
        if algorithm == "secp256k1" and coincurve is not None:
            try:
                return Exchange._ecdsa_secp256k1_coincurve(
                    request, secret, hash, fixed_length
                )
            except Exception:
        ...
        if secret.find(b"-----BEGIN EC PRIVATE KEY-----") > -1:
            ...
            key = ecdsa.SigningKey.from_string(
                base64.b16decode(secret, casefold=True), curve=curve_info[0]
            )
        r_binary, s_binary, v = key.sign_digest_deterministic(
            digest,
            hashfunc=hash_function,
            sigencode=ecdsa.util.sigencode_strings_canonize,
        )
        r_int, s_int = ecdsa.util.sigdecode_strings(
            (r_binary, s_binary), key.privkey.order
        )
        counter = 0
        minimum_size = (1 << (8 * 31)) - 1
        half_order = key.privkey.order / 2
        while fixed_length and (
            r_int > half_order or r_int <= minimum_size or s_int <= minimum_size
        ):
            r_binary, s_binary, v = key.sign_digest_deterministic(
                digest,
                hashfunc=hash_function,
            ...
            counter += 1
        r, s = (
            Exchange.decode(base64.b16encode(r_binary)).lower(),
            Exchange.decode(base64.b16encode(s_binary)).lower(),
        )
        return {
            "r": r,
            "s": s,
    ...
    def _ecdsa_secp256k1_coincurve(request, secret, hash=None, fixed_length=False):
        ...
        # Handle PEM format
        if secret.find(b"-----BEGIN EC PRIVATE KEY-----") > -1:
            secret = base64.b16decode(
                secret.replace(b"-----BEGIN EC PRIVATE KEY-----", b"")
                .replace(b"-----END EC PRIVATE KEY-----", b"")
                .replace(b"\n", b"")
                .replace(b"\r", b""),
                casefold=True,
            )
    ...
    def eddsa(request, secret, curve="ed25519", url_encode=False):
        if isinstance(secret, str):
            secret = Exchange.encode(secret)
        private_key = (
            ed25519.Ed25519PrivateKey.from_private_bytes(secret)
            if len(secret) == 32
            else load_pem_private_key(secret, None)
        )
    ...
    def check_required_dependencies(self):
        if self.requiresEddsa and eddsa is None:
            raise NotSupported(
                self.id
                + " Eddsa functionality requires python-axolotl-curve25519, install with `pip install python-axolotl-curve25519==0.4.1.post2`: https://github.com/tgalal/python-axolotl-curve25519"
            )
        ...
        if price is None:
            if type == "limit":
                raise ArgumentsRequired(
                    self.id
                    + " create_order() requires a price argument for a limit order"
                )
        if amount <= 0:
    ...
    def get_zk_contract_signature_obj(self, seeds: str, params={}):
        ...
        nonceInt = int(
            self.remove0x_prefix(self.hash(self.encode(slotId), "sha256", "hex")), 16
        )
        ...
        builder = zklink_sdk.ContractBuilder(
            ...
            int(slotId),
            int(nonce),
            int(self.safe_number(params, "pairId")),
            sizeStr.__str__(),
            priceStr.__str__(),
            self.safe_string(params, "direction") == "BUY",
            int(takerFeeRateStr),
            int(makerFeeRateStr),
            False,
        )
    ...
    def safe_bool_n(
        self, dictionaryOrList, keys: List[IndexType], defaultValue: bool = None
    ):
        """
        @ignore
    ...
    def safe_bool_2(
        self, dictionary, key1: IndexType, key2: IndexType, defaultValue: bool = None
    ):
        """
        @ignore
               safely extract boolean value from dictionary or list
    ...
    def safe_dict_n(
        self, dictionaryOrList, keys: List[IndexType], defaultValue: dict = None
    ):
        """
        @ignore
    ...
    def safe_dict_2(
        self, dictionary, key1: IndexType, key2: str, defaultValue: dict = None
    ):
        """
        @ignore
               safely extract a dictionary from dictionary or list
    ...
    def safe_list_n(
        self, dictionaryOrList, keys: List[IndexType], defaultValue: List[Any] = None
    ):
        """
        @ignore
    ...
    def safe_list(
        self, dictionaryOrList, key: IndexType, defaultValue: List[Any] = None
    ):
        """
        @ignore
               safely extract an Array from dictionary or list
    ...
    def check_proxy_url_settings(
        self, url: Str = None, method: Str = None, headers=None, body=None
    ):
        usedProxies = []
        proxyUrl = None
    ...
    def check_proxy_settings(
        ...
        if ishttpProxyCallbackDefined or ishttp_proxy_callback_defined:
            ...
            httpProxy = (
                self.httpProxyCallback(url, method, headers, body)
                if ishttpProxyCallbackDefined
                else self.http_proxy_callback(url, method, headers, body)
            )
        # httpsProxy
        ...
        ishttps_proxy_callback_defined = self.value_is_defined(
            self.https_proxy_callback
        )
        if ishttpsProxyCallbackDefined or ishttps_proxy_callback_defined:
            usedProxies.append("httpsProxyCallback")
            httpsProxy = (
                self.httpsProxyCallback(url, method, headers, body)
                if ishttpsProxyCallbackDefined
                else self.https_proxy_callback(url, method, headers, body)
            )
        # socksProxy
        ...
        issocks_proxy_callback_defined = self.value_is_defined(
            self.socks_proxy_callback
        )
        if issocksProxyCallbackDefined or issocks_proxy_callback_defined:
            usedProxies.append("socksProxyCallback")
            socksProxy = (
                self.socksProxyCallback(url, method, headers, body)
                if issocksProxyCallbackDefined
                else self.socks_proxy_callback(url, method, headers, body)
            )
        # check
        length = len(usedProxies)
        if length > 1:
    ...
    def check_ws_proxy_settings(self):
        ...
        if isWsSocksProxyDefined or is_ws_socks_proxy_defined:
            ...
            wsSocksProxy = (
                self.wsSocksProxy if (isWsSocksProxyDefined) else self.ws_socks_proxy
            )
        # check
        length = len(usedProxies)
        if length > 1:
    ...
    def check_conflicting_proxies(self, proxyAgentSet, proxyUrlSet):
        if proxyAgentSet and proxyUrlSet:
            raise InvalidProxySettings(
                self.id
                + " you have multiple conflicting proxy settings, please use only one from : proxyUrl, httpProxy, httpsProxy, socksProxy"
            )
    ...
    def check_address(self, address: Str = None):
        ...
        if (
            length == 1
            or len(address) < self.minFundingAddressLength
            or address.find(" ") > -1
        ):
            raise InvalidAddress(
                self.id
                + " address is invalid or has less than "
                + str(self.minFundingAddressLength)
                + ' characters: "'
                + str(address)
                + '"'
            )
    ...
    def filter_by_value_since_limit(
        ...
        if valueIsDefined or sinceIsDefined:
            ...
            for i in range(0, len(parsedArray)):
                ...
                entryKeyGESince = (
                    (entryKeyValue) and (since is not None) and (entryKeyValue >= since)
                )
    ...
    def enable_demo_trading(self, enable: bool):
        ...
        if self.isSandboxModeEnabled:
            raise NotSupported(
                self.id
                + " demo trading does not support in sandbox environment. Please check https://www.binance.com/en/support/faq/detail/9be58f73e5e14338809e3b705b9687dd to see the differences"
            )
        if enable:
    ...
    def watch_liquidations_for_symbols(
        ...
        raise NotSupported(
            self.id + " watchLiquidationsForSymbols() is not supported yet"
        )
    ...
    def watch_my_liquidations(
        ...
        if self.has["watchMyLiquidationsForSymbols"]:
            return self.watch_my_liquidations_for_symbols(
    ...
    def watch_my_liquidations_for_symbols(
        ...
        raise NotSupported(
            self.id + " watchMyLiquidationsForSymbols() is not supported yet"
        )
    ...
    def un_watch_order_book_for_symbols(self, symbols: List[str], params={}):
        raise NotSupported(
            self.id + " unWatchOrderBookForSymbols() is not supported yet"
        )
    ...
    def fetch_long_short_ratio_history(
        ...
        raise NotSupported(
            self.id + " fetchLongShortRatioHistory() is not supported yet"
        )
    ...
    def fetch_margin_adjustment_history(
        ...
        """
        raise NotSupported(
            self.id + " fetchMarginAdjustmentHistory() is not supported yet"
        )
    ...
    def fetch_deposit_addresses_by_network(self, code: str, params={}):
        raise NotSupported(
            self.id + " fetchDepositAddressesByNetwork() is not supported yet"
        )
    ...
    def parse_to_numeric(self, number):
        stringVersion = self.number_to_string(
            number
        )  # self will convert 1.0 and 1 to "1" and 1.1 to "1.1"
        # keep self in mind:
        # in JS:     1 == 1.0 is True
    ...
    def safe_number_omit_zero(
        self, obj: object, key: IndexType, defaultValue: Num = None
    ):
    ...
    def features_generator(self):
        ...
        for i in range(0, len(unifiedMarketTypes)):
            ...
            if not (marketType in initialFeatures):
                ...
                if marketType == "spot":
                    self.features[marketType] = self.features_mapper(
                        initialFeatures, marketType, None
                    )
                else:
    ...
    def features_mapper(
        self, initialFeatures: Any, marketType: Str, subType: Str = None
    ):
        featuresObj = (
            initialFeatures[marketType][subType]
        ...
        if "createOrder" in featuresObj:
            value = self.safe_dict(
                featuresObj["createOrder"], "attachedStopLossTakeProfit"
            )
    ...
    def feature_value_by_type(
        ...
        if not (marketType in self.features):
            return (
                defaultValue
            )  # unsupported marketType, check "exchange.features" for details
        ...
        if subType is None:
            ...
            if not (subType in self.features[marketType]):
                return (
                    defaultValue
                )  # unsupported subType, check "exchange.features" for details
        ...
        if not (methodName in methodsContainer):
            return (
                defaultValue
            )  # unsupported method, check "exchange.features" for details')
        ...
        if not (parentKey in methodDict):
            return (
                defaultValue
            )  # unsupported paramName, check "exchange.features" for details')
        ...
        if dictionary is None:
            ...
            if not (subKey in methodDict[parentKey]):
                return (
                    defaultValue
                )  # unsupported subKey, check "exchange.features" for details
    ...
    def create_networks_by_id_object(self):
        ...
        networkIdsToCodesGenerated = self.invert_flat_string_dictionary(
            self.safe_value(self.options, "networks", {})
        )  # invert defined networks dictionary
        self.options["networksById"] = self.extend(
            networkIdsToCodesGenerated,
            self.safe_value(self.options, "networksById", {}),
        ...
        if length != 0:
            for i in range(0, length):
                ...
                active = self.safe_bool(
                    currency["networks"][key], "active"
                )  # dict might have been updated on above lines, so access directly instead of `network` variable
                ...
                if limitsDepositMinMain is None or Precise.string_lt(
                    ...
                    currency["limits"]["deposit"]["min"] = self.parse_number(
                        limitsDepositMin
                    )
                # find max
                if limitsDepositMaxMain is None or Precise.string_gt(
                    ...
                    currency["limits"]["deposit"]["max"] = self.parse_number(
                        limitsDepositMax
                    )
                # withdrawals
                ...
                if limitsWithdrawMinMain is None or Precise.string_lt(
                    ...
                    currency["limits"]["withdraw"]["min"] = self.parse_number(
                        limitsWithdrawMin
                    )
                # find max
                if limitsWithdrawMaxMain is None or Precise.string_gt(
                    ...
                    currency["limits"]["withdraw"]["max"] = self.parse_number(
                        limitsWithdrawMax
                    )
        return self.extend(
            {
        ...
        if numCurrencies > 0:
            ...
            for i in range(0, len(values)):
                ...
                defaultCurrencyPrecision = (
                    8
                    if (self.precisionMode == DECIMAL_PLACES)
                    else self.parse_number("1e-8")
                )
            ...
            self.baseCurrencies = self.map_to_safe_map(
                self.index_by(baseCurrencies, "code")
            )
            self.quoteCurrencies = self.map_to_safe_map(
                self.index_by(quoteCurrencies, "code")
            )
                    ...
                    if self.precisionMode == TICK_SIZE:
                        highestPrecisionCurrency = (
                            ...
                            if (
                                ...
                                < highestPrecisionCurrency["precision"]
                            )
            ...
            self.currencies = self.map_to_safe_map(
                self.deep_extend(
                    self.currencies, self.index_by(sortedCurrencies, "code")
                )
            )
    ...
    def set_markets_from_exchange(self, sourceExchange):
        ...
        sourceExchangeHelpers = self.safe_list(
            sourceExchange.options, "marketHelperProps", []
        )
    ...
    def get_describe_for_extended_ws_exchange(
        self, currentRestInstance: Any, parentRestInstance: Any, wsBaseDescribe: dict
    ):
    ...
    def safe_balance(self, balance: dict):
        balances = self.omit(
            balance, ["info", "timestamp", "datetime", "free", "used", "total"]
        )
    ...
    def safe_order(self, order: dict, market: Market = None):
        # Micro-optimization: cache commonly used methods/attrs to locals, use in loop
        ...
        omit_zero = self.omit_zero
        number_to_string = self.number_to_string
        parse_number = self.parse_number
        reduce_fees_by_currency = self.reduce_fees_by_currency
        deep_extend = self.deep_extend
        safe_bool = self.safe_bool
        parse_trades = self.parse_trades
        parse_bool = self.safe_bool
        Precise_add = Precise.string_add
        Precise_sub = Precise.string_sub
        Precise_mul = Precise.string_mul
        Precise_div = Precise.string_div
        Precise_gt = Precise.string_gt
        Precise_equals = Precise.string_equals
        extend = self.extend

        amount = omit_zero(safe_string(order, "amount"))
        remaining = safe_string(order, "remaining")
        filled = safe_string(order, "filled")
        cost = safe_string(order, "cost")
        average = omit_zero(safe_string(order, "average"))
        price = omit_zero(safe_string(order, "price"))
        lastTradeTimeTimestamp = safe_integer(order, "lastTradeTimestamp")
        symbol = safe_string(order, "symbol")
        side = safe_string(order, "side")
        status = safe_string(order, "status")
        parseFilled = filled is None
        parseCost = cost is None
        parseLastTradeTimeTimestamp = lastTradeTimeTimestamp is None
        fee = safe_value(order, "fee")
        parseFee = fee is None
        parseFees = safe_value(order, "fees") is None
        parseSymbol = symbol is None
        parseSide = side is None
        shouldParseFees = parseFee or parseFees
        fees = safe_list(order, "fees", [])
        trades = []
        isTriggerOrSLTpOrder = (
            safe_string(order, "triggerPrice") is not None
            or (safe_string(order, "stopLossPrice") is not None)
        ) or (safe_string(order, "takeProfitPrice") is not None)
        if parseFilled or parseCost or shouldParseFees:
            rawTrades = safe_value(order, "trades", trades)
            firstTrade = safe_value(rawTrades, 0)
            tradesAreParsed = (
                (firstTrade is not None)
                and ("info" in firstTrade)
                and ("id" in firstTrade)
            )
            if not tradesAreParsed:
                trades = parse_trades(rawTrades, market)
            else:
                trades = rawTrades
            tradesLength = len(trades) if isinstance(trades, list) else 0
            if tradesLength > 0:
                if order.get("symbol", None) is None:
                    order["symbol"] = trades[0]["symbol"]
                if order.get("side", None) is None:
                    order["side"] = trades[0]["side"]
                if order.get("type", None) is None:
                    order["type"] = trades[0]["type"]
                if order.get("id", None) is None:
                ...
                if parseCost:
                    cost = "0"
                for trade in trades:
                    tradeAmount = safe_string(trade, "amount")
                    if parseFilled and tradeAmount is not None:
                        filled = Precise_add(filled, tradeAmount)
                    tradeCost = safe_string(trade, "cost")
                    if parseCost and tradeCost is not None:
                        cost = Precise_add(cost, tradeCost)
                    if parseSymbol:
                        symbol = safe_string(trade, "symbol")
                    if parseSide:
                        side = safe_string(trade, "side")
                    tradeTimestamp = safe_value(trade, "timestamp")
                    if parseLastTradeTimeTimestamp and (tradeTimestamp is not None):
                        if lastTradeTimeTimestamp is None:
                            lastTradeTimeTimestamp = tradeTimestamp
                        else:
                            lastTradeTimeTimestamp = max(
                                lastTradeTimeTimestamp, tradeTimestamp
                            )
                    if shouldParseFees:
                        tradeFees = safe_value(trade, "fees")
                        if tradeFees is not None:
                            for tradeFee in tradeFees:
                                fees.append(extend({}, tradeFee))
                        else:
                            tradeFee = safe_value(trade, "fee")
                            if tradeFee is not None:
                                fees.append(extend({}, tradeFee))
        if shouldParseFees:
            reducedFees = reduce_fees_by_currency(fees) if self.reduceFees else fees
            reducedLength = len(reducedFees)
            for rf in reducedFees:
                rf["cost"] = self.safe_number(rf, "cost")
                if "rate" in rf:
                    rf["rate"] = self.safe_number(rf, "rate")
            if not parseFee and (reducedLength == 0):
                feeCopy = deep_extend(fee)
        ...
        if amount is None:
            ...
            if filled is not None and remaining is not None:
                amount = Precise_add(filled, remaining)
            elif status == "closed":
                amount = filled
        if filled is None:
            if amount is not None and remaining is not None:
                filled = Precise_sub(amount, remaining)
            elif status == "closed" and amount is not None:
                filled = amount
        if remaining is None:
            if amount is not None and filled is not None:
                remaining = Precise_sub(amount, filled)
            elif status == "closed":
                remaining = "0"
        inverse = safe_bool(market, "inverse", False)
        contractSize = number_to_string(safe_value(market, "contractSize", 1))
        # inverse
        ...
        if average is None:
            if (filled is not None) and (cost is not None) and Precise_gt(filled, "0"):
                filledTimesContractSize = Precise_mul(filled, contractSize)
                if inverse:
                    average = Precise_div(filledTimesContractSize, cost)
                else:
                    average = Precise_div(cost, filledTimesContractSize)
        # similarly
        # inverse
        ...
        if parseCost and (filled is not None) and costPriceExists:
            ...
            filledTimesContractSize = Precise_mul(filled, contractSize)
            if inverse:
                cost = Precise_div(filledTimesContractSize, multiplyPrice)
            else:
                cost = Precise_mul(filledTimesContractSize, multiplyPrice)
        orderType = safe_value(order, "type")
        emptyPrice = (price is None) or Precise_equals(price, "0")
        if emptyPrice and (orderType == "market"):
            price = average
        # Mutate trades and tradeFees
        for entry in trades:
            ...
            entryFees = safe_list(entry, "fees", [])
            for ef in entryFees:
                ef["cost"] = self.safe_number(ef, "cost")
            entry["fees"] = entryFees
            entry["fee"] = tradeFee
        timeInForce = safe_string(order, "timeInForce")
        postOnly = safe_value(order, "postOnly")
        # timeInForceHandling
        if timeInForce is None:
            if not isTriggerOrSLTpOrder and (safe_string(order, "type") == "market"):
                timeInForce = "IOC"
        ...
        timestamp = safe_integer(order, "timestamp")
        lastUpdateTimestamp = safe_integer(order, "lastUpdateTimestamp")
        datetime_str = safe_string(order, "datetime")
        if datetime_str is None:
            datetime_str = self.iso8601(timestamp)
        triggerPrice = parse_number(safe_string_2(order, "triggerPrice", "stopPrice"))
        takeProfitPrice = parse_number(safe_string(order, "takeProfitPrice"))
        stopLossPrice = parse_number(safe_string(order, "stopLossPrice"))
        # Compose output, use locals
        return self.extend(
            order,
            {
                "id": safe_string(order, "id"),
                "clientOrderId": safe_string(order, "clientOrderId"),
                "timestamp": timestamp,
                "datetime": datetime_str,
                "symbol": symbol,
                "type": safe_string(order, "type"),
                "side": side,
                ...
                "average": parse_number(average),
                "filled": parse_number(filled),
                "remaining": parse_number(remaining),
                "timeInForce": timeInForce,
                "postOnly": postOnly,
                "trades": trades,
                "reduceOnly": safe_value(order, "reduceOnly"),
    ...
    def parse_orders(
        ...
        if isinstance(orders, list):
            ...
            for i in range(0, len(ids)):
                ...
                parsedOrder = self.parse_order(
                    idExtended, market
                )  # don't  inline these calls
    ...
    def calculate_fee_with_rate(
        ...
        rate = (
            ...
            else self.safe_string(market, takerOrMaker)
        )
    ...
    def create_ccxt_trade_id(
        self, timestamp=None, side=None, amount=None, price=None, takerOrMaker=None
    ):
    ...
    def parsed_fee_and_fees(self, container: Any):
        ...
        if shouldParseFees:
            ...
            reducedFees = (
                self.reduce_fees_by_currency(fees) if self.reduceFees else fees
            )
            reducedLength = len(reducedFees)
        ...
        for i in range(0, len(fees)):
            ...
            if feeCurrencyCode is not None:
                ...
                if rateKey in reduced[feeCurrencyCode]:
                    reduced[feeCurrencyCode][rateKey]["cost"] = Precise.string_add(
                        reduced[feeCurrencyCode][rateKey]["cost"], cost
                    )
                else:
    ...
    def safe_ticker(self, ticker: dict, market: Market = None):
        ...
        if change is not None:
            ...
            if close is None and average is not None:
                ...
                denominator = Precise.string_add(
                    "2", Precise.string_div(percentage, "100")
                )
                calcOpen = (
                    open
                    if (open is not None)
                    else Precise.string_div(openAddClose, denominator)
                )
                close = Precise.string_mul(
                    calcOpen,
                    Precise.string_add("1", Precise.string_div(percentage, "100")),
                )
            if open is None and close is not None:
        ...
        if change is None:
            if close is not None and open is not None:
                ...
                change = Precise.string_mul(
                    Precise.string_div(percentage, "100"),
                    Precise.string_div(close, "100"),
                )
        ...
        if open is not None:
            ...
            if average is None and close is not None:
                ...
                average = Precise.string_div(
                    Precise.string_add(open, close), "2", precision
                )
        ...
        return self.extend(
            ticker,
            {
                "bid": self.parse_number(
                    self.omit_zero(self.safe_string(ticker, "bid"))
                ),
                "bidVolume": self.safe_number(ticker, "bidVolume"),
                "ask": self.parse_number(
                    self.omit_zero(self.safe_string(ticker, "ask"))
                ),
                "askVolume": self.safe_number(ticker, "askVolume"),
                "high": self.parse_number(
                    self.omit_zero(self.safe_string(ticker, "high"))
                ),
                "low": self.parse_number(
                    self.omit_zero(self.safe_string(ticker, "low"))
                ),
    ...
    def convert_trading_view_to_ohlcv(
        ...
        for i in range(0, len(timestamps)):
            result.append(
                [
                    self.safe_integer(timestamps, i)
                    if ms
                    else self.safe_timestamp(timestamps, i),
        ...
        try:
            ...
            if returnAsJson and (isinstance(content, str)):
                ...
                if jsoned:
                    return (
                        jsoned
                    )  # if parsing was not successfull, exception should be thrown
                else:
            ...
            errorMessage = (
                ...
                + " "
                + method
                + "() failed to fetch correct data from website. Probably webpage markup has been changed, breaking the page custom parser."
            )
        if muteOnFailure:
            return None
        else:
    ...
    def market_symbols(
        ...
        if symbols is None:
            if not allowEmpty:
                raise ArgumentsRequired(
                    self.id + " empty list of symbols is not supported"
                )
            return symbols
        ...
        for i in range(0, len(symbols)):
            ...
            if sameTypeOnly and (marketType is not None):
                if market["type"] != marketType:
                    raise BadRequest(
                        self.id
                        + " symbols must be of the same type, either "
                        + marketType
                        + " or "
                        + market["type"]
                        + "."
            ...
            if sameSubTypeOnly and (isLinearSubType is not None):
                if market["linear"] != isLinearSubType:
                    raise BadRequest(
                        self.id
                        + " symbols must be of the same subType, either linear or inverse."
                    )
            if type is not None and market["type"] != type:
                raise BadRequest(
                    self.id
                    + " symbols must be of the same type "
                    + type
    ...
    def parse_bids_asks(
        ...
        for i in range(0, len(bidasks)):
            result.append(
                self.parse_bid_ask(bidasks[i], priceKey, amountKey, countOrIdKey)
            )
        return result
    ...
    def network_code_to_id(self, networkCode: str, currencyCode: Str = None):
        ...
        if networkId is None:
            if currencyCode is None:
                ...
                defaultNetworkCodeReplacements = self.safe_value(
                    self.options, "defaultNetworkCodeReplacements", {}
                )
                if currencyCode in defaultNetworkCodeReplacements:
                    # if there is a replacement for the passed networkCode, then we use it to find network-id in `options->networks` object
                    replacementObject = defaultNetworkCodeReplacements[
                        currencyCode
                    ]  # i.e. {'ERC20': 'ETH'}
    ...
    def network_id_to_code(self, networkId: Str = None, currencyCode: Str = None):
        ...
        if currencyCode is not None:
            defaultNetworkCodeReplacements = self.safe_dict(
                self.options, "defaultNetworkCodeReplacements", {}
            )
            if currencyCode in defaultNetworkCodeReplacements:
                replacementObject = self.safe_dict(
                    defaultNetworkCodeReplacements, currencyCode, {}
                )
                networkCode = self.safe_string(
                    replacementObject, networkCode, networkCode
                )
        return networkCode
    ...
    def select_network_code_from_unified_networks(
        self, currencyCode, networkCode, indexedNetworkEntries
    ):
    ...
    def select_network_key_from_networks(
        ...
        if networkCode is not None:
            ...
            if responseNetworksLength == 0:
                raise NotSupported(
                    self.id + " - no networks were returned for " + currencyCode
                )
            else:
    ...
    def safe_number_2(
        self, dictionary: object, key1: IndexType, key2: IndexType, d=None
    ):
    ...
    def parse_leverage_tiers(
        self, response: Any, symbols: List[str] = None, marketIdKey=None
    ):
    ...
    def load_trading_limits(self, symbols: Strings = None, reload=False, params={}):
        if self.has["fetchTradingLimits"]:
            if reload or not ("limitsLoaded" in self.options):
                ...
                for i in range(0, len(symbols)):
                    symbol = symbols[i]
                    self.markets[symbol] = self.deep_extend(
                        self.markets[symbol], response[symbol]
                    )
    ...
    def parse_transactions(
        ...
        for i in range(0, len(transactions)):
            transaction = self.extend(
                self.parse_transaction(transactions[i], currency), params
            )
    ...
    def handle_param_string(
        self, params: object, paramName: str, defaultValue: Str = None
    ):
    ...
    def handle_param_integer(
        self, params: object, paramName: str, defaultValue: Int = None
    ):
    ...
    def handle_param_bool(
        self, params: object, paramName: str, defaultValue: Bool = None
    ):
    ...
    def handle_request_network(
        ...
        if networkCode is not None:
            request[exchangeSpecificKey] = self.network_code_to_id(
                networkCode, currencyCode
            )
        elif isRequired:
            raise ArgumentsRequired(
                self.id + ' - "network" param is required for self request'
            )
    ...
    def get_symbols_for_market_type(
        self,
        marketType: Str = None,
        subType: Str = None,
        symbolWithActiveStatus: bool = True,
        symbolWithUnknownStatus: bool = True,
    ):
        ...
        filteredMarkets = self.filter_by_array(
            filteredMarkets, "active", activeStatuses, False
        )
    ...
    def fetch2(
        ...
        if self.enableRateLimit:
            ...
            self.throttle(cost)
        retries = None
        retries, params = self.handle_option_and_params(
            params, path, "maxRetriesOnFailure", 0
        )
        retryDelay = None
        retryDelay, params = self.handle_option_and_params(
            params, path, "maxRetriesOnFailureDelay", 0
        )
        ...
        return (
            None
        )  # self line is never reached, but exists for c# value return requirement
    ...
    def build_ohlcvc(
        ...
        for i in range(0, oldest):
            ...
            if isFirstCandle or openingTime >= self.sum(
                ohlcvs[candle][i_timestamp], ms
            ):
                ...
                ohlcvs[candle][i_volume] = self.sum(
    ...
    def parse_trading_view_ohlcv(
        self, ohlcvs, market=None, timeframe="1m", since: Int = None, limit: Int = None
    ):
    ...
    def edit_order_with_client_order_id(
        self,
        clientOrderId: str,
        symbol: str,
        type: OrderType,
        ...
        return self.edit_order(
            ...
            amount,
            price,
            self.extend({"clientOrderId": clientOrderId}, params),
        )
    ...
    def watch_positions(
        self, symbols: Strings = None, since: Int = None, limit: Int = None, params={}
    ):
    ...
    def safe_currency(self, currencyId: Str, currency: Currency = None):
        if (currencyId is None) and (currency is not None):
            return currency
        if (
            (self.currencies_by_id is not None)
            and (currencyId in self.currencies_by_id)
            and (self.currencies_by_id[currencyId] is not None)
        ):
            ...
            if self.requiredCredentials[key] and not getattr(self, key):
                if error:
                    raise AuthenticationError(
                        self.id + ' requires "' + key + '" credential'
                    )
                else:
                    return False
        return True
    ...
    def fetch_deposit_withdraw_fee(self, code: str, params={}):
        if not self.has["fetchDepositWithdrawFees"]:
            raise NotSupported(
                self.id + " fetchDepositWithdrawFee() is not supported yet"
            )
    ...
    def get_supported_mapping(self, key, mapping={}):
        if key in mapping:
            ...
            raise NotSupported(
                self.id + " " + key + " does not have a value in mapping"
            )
    ...
    def fetch_isolated_borrow_rate(self, symbol: str, params={}):
        ...
        if not self.has["fetchBorrowRates"]:
            raise NotSupported(
                self.id + " fetchIsolatedBorrowRate() is not supported yet"
            )
    ...
    def handle_option_and_params(
        self, params: object, methodName: str, optionName: str, defaultValue=None
    ):
        ...
        if value is not None:
            ...
            methodName, params = self.handle_param_string(
            ...
            if exchangeWideMethodOptions is not None:
                ...
                value = self.safe_value_2(
                    exchangeWideMethodOptions, optionName, defaultOptionName
                )
            if value is None:
    ...
    def handle_option_and_params_2(
        ...
        value2 = None
        value2, params = self.handle_option_and_params(
            params, methodName1, optionName2, defaultValue
        )
    ...
    def handle_market_type_and_params(
        self, methodName: str, market: Market = None, params={}, defaultValue=None
    ):
        """
        @ignore
        ...
        if methodOptions is not None:
            if isinstance(methodOptions, str):
                ...
                typeFromMethod = self.safe_string_2(
                    methodOptions, "defaultType", "type"
                )
    ...
    def handle_sub_type_and_params(
        self, methodName: str, market=None, params={}, defaultValue=None
    ):
        subType = None
        ...
        if subTypeInParams is not None:
            ...
            if subType is None:
                values = self.handle_option_and_params(
                    {}, methodName, "subType", defaultValue
                )  # no need to re-test params here
                subType = values[0]
        return [subType, params]

    def handle_margin_mode_and_params(
        self, methodName: str, params={}, defaultValue=None
    ):
        """
        @ignore
               :param dict [params]: extra parameters specific to the exchange API endpoint
        ...
        """
        return self.handle_option_and_params(
            params, methodName, "marginMode", defaultValue
        )
    ...
    def fetch_ticker(self, symbol: str, params={}):
        if self.has["fetchTickers"]:
            ...
            if ticker is None:
                raise NullResponse(
                    self.id + " fetchTickers() could not find a ticker for " + symbol
                )
            else:
                return ticker
        else:
    ...
    def fetch_mark_price(self, symbol: str, params={}):
        if self.has["fetchMarkPrices"]:
            ...
            if ticker is None:
                raise NullResponse(
                    self.id + " fetchMarkPrices() could not find a ticker for " + symbol
                )
            else:
                return ticker
        else:
    ...
    def fetch_ticker_ws(self, symbol: str, params={}):
        if self.has["fetchTickersWs"]:
            ...
            if ticker is None:
                raise NullResponse(
                    self.id + " fetchTickerWs() could not find a ticker for " + symbol
                )
            else:
                return ticker
        else:
    ...
    def fetch_order_with_client_order_id(
        self, clientOrderId: str, symbol: Str = None, params={}
    ):
        """
    ...
    def create_trailing_amount_order(
        ...
        if trailingAmount is None:
            raise ArgumentsRequired(
                self.id
                + " createTrailingAmountOrder() requires a trailingAmount argument"
            )
        ...
        if self.has["createTrailingAmountOrder"]:
            return self.create_order(symbol, type, side, amount, price, params)
        raise NotSupported(
            self.id + " createTrailingAmountOrder() is not supported yet"
        )
    ...
    def create_trailing_amount_order_ws(
        ...
        if trailingAmount is None:
            raise ArgumentsRequired(
                self.id
                + " createTrailingAmountOrderWs() requires a trailingAmount argument"
            )
        params["trailingAmount"] = trailingAmount
        if trailingTriggerPrice is not None:
            params["trailingTriggerPrice"] = trailingTriggerPrice
        if self.has["createTrailingAmountOrderWs"]:
            return self.create_order_ws(symbol, type, side, amount, price, params)
        raise NotSupported(
            self.id + " createTrailingAmountOrderWs() is not supported yet"
        )
    ...
    def create_trailing_percent_order(
        ...
        if trailingPercent is None:
            raise ArgumentsRequired(
                self.id
                + " createTrailingPercentOrder() requires a trailingPercent argument"
            )
        ...
        if self.has["createTrailingPercentOrder"]:
            return self.create_order(symbol, type, side, amount, price, params)
        raise NotSupported(
            self.id + " createTrailingPercentOrder() is not supported yet"
        )

    def create_trailing_percent_order_ws(
        self,
        symbol: str,
        type: OrderType,
        side: OrderSide,
        ...
        if trailingPercent is None:
            raise ArgumentsRequired(
                self.id
                + " createTrailingPercentOrderWs() requires a trailingPercent argument"
            )
        params["trailingPercent"] = trailingPercent
        if trailingTriggerPrice is not None:
            params["trailingTriggerPrice"] = trailingTriggerPrice
        if self.has["createTrailingPercentOrderWs"]:
            return self.create_order_ws(symbol, type, side, amount, price, params)
        raise NotSupported(
            self.id + " createTrailingPercentOrderWs() is not supported yet"
        )
    ...
    def create_market_order_with_cost(
        ...
        :returns dict: an `order structure <https://docs.ccxt.com/#/?id=order-structure>`
        """
        if self.has["createMarketOrderWithCost"] or (
            self.has["createMarketBuyOrderWithCost"]
            and self.has["createMarketSellOrderWithCost"]
        ):
            return self.create_order(symbol, "market", side, cost, 1, params)
        raise NotSupported(
            self.id + " createMarketOrderWithCost() is not supported yet"
        )
    ...
    def create_market_buy_order_with_cost(self, symbol: str, cost: float, params={}):
        ...
        if (
            ...
            or self.has["createMarketBuyOrderWithCost"]
        ):
            return self.create_order(symbol, "market", "buy", cost, 1, params)
        raise NotSupported(
            self.id + " createMarketBuyOrderWithCost() is not supported yet"
        )
    ...
    def create_market_sell_order_with_cost(self, symbol: str, cost: float, params={}):
        ...
        """
        if (
            self.options["createMarketSellOrderRequiresPrice"]
            or self.has["createMarketSellOrderWithCost"]
        ):
            return self.create_order(symbol, "market", "sell", cost, 1, params)
        raise NotSupported(
            self.id + " createMarketSellOrderWithCost() is not supported yet"
        )
    ...
    def create_market_order_with_cost_ws(
        ...
        :returns dict: an `order structure <https://docs.ccxt.com/#/?id=order-structure>`
        """
        if self.has["createMarketOrderWithCostWs"] or (
            self.has["createMarketBuyOrderWithCostWs"]
            and self.has["createMarketSellOrderWithCostWs"]
        ):
            return self.create_order_ws(symbol, "market", side, cost, 1, params)
        raise NotSupported(
            self.id + " createMarketOrderWithCostWs() is not supported yet"
        )
    ...
    def create_take_profit_order_ws(
        self,
        symbol: str,
        type: OrderType,
        side: OrderSide,
        ...
        if takeProfitPrice is None:
            raise ArgumentsRequired(
                self.id
                + " createTakeProfitOrderWs() requires a takeProfitPrice argument"
            )
        params["takeProfitPrice"] = takeProfitPrice
    ...
    def create_order_with_take_profit_and_stop_loss(
        ...
        if self.has["createOrderWithTakeProfitAndStopLoss"]:
            return self.create_order(symbol, type, side, amount, price, params)
        raise NotSupported(
            self.id + " createOrderWithTakeProfitAndStopLoss() is not supported yet"
        )
    ...
    def create_order_with_take_profit_and_stop_loss_ws(
        self,
        symbol: str,
        type: OrderType,
        side: OrderSide,
    ...
    def cancel_orders_with_client_order_ids(
        self, clientOrderIds: List[str], symbol: Str = None, params={}
    ):
        """
        create a market order by providing the symbol, side and cost
    ...
    def fetch_orders(
        ...
        if self.has["fetchOpenOrders"] and self.has["fetchClosedOrders"]:
            raise NotSupported(
                self.id
                + " fetchOrders() is not supported yet, consider using fetchOpenOrders() and fetchClosedOrders() instead"
            )
    ...
    def fetch_canceled_and_closed_orders(
        ...
        raise NotSupported(
            self.id + " fetchCanceledAndClosedOrders() is not supported yet"
        )
    ...
    def market(self, symbol: str):
        ...
        if symbol in self.markets:
            ...
            defaultType = self.safe_string_2(
                self.options, "defaultType", "defaultSubType", "spot"
            )
            for i in range(0, len(markets)):
                market = markets[i]
    ...
    def create_expired_option_market(self, symbol: str):
        raise NotSupported(
            self.id + " createExpiredOptionMarket() is not supported yet"
        )

    def is_leveraged_currency(
        self, currencyCode, checkBaseCoin: Bool = False, existingCurrencies: dict = None
    ):
        leverageSuffixes = [
            "2L",
            "2S",
    ...
    def currency_to_precision(self, code: str, fee, networkCode=None):
        ...
        if precision is None:
            ...
            roundingMode = self.safe_integer(
                self.options, "currencyToPrecisionRoundingMode", ROUND
            )
            return self.decimal_to_precision(
    ...
    def safe_number_n(
        self, obj: object, arr: List[IndexType], defaultNumber: Num = None
    ):
    ...
    def fetch_market_leverage_tiers(self, symbol: str, params={}):
        if self.has["fetchLeverageTiers"]:
            ...
            raise NotSupported(
                self.id + " fetchMarketLeverageTiers() is not supported yet"
            )
    ...
    def create_post_only_order_ws(
        ...
        if not self.has["createPostOnlyOrderWs"]:
            raise NotSupported(
                self.id + " createPostOnlyOrderWs() is not supported yet"
            )
        query = self.extend(params, {"postOnly": True})
    ...
    def create_reduce_only_order(
        ...
        if not self.has["createReduceOnlyOrder"]:
            raise NotSupported(
                self.id + " createReduceOnlyOrder() is not supported yet"
            )
    ...
    def create_reduce_only_order_ws(
        ...
        if not self.has["createReduceOnlyOrderWs"]:
            raise NotSupported(
                self.id + " createReduceOnlyOrderWs() is not supported yet"
            )
        query = self.extend(params, {"reduceOnly": True})
        return self.create_order_ws(symbol, type, side, amount, price, query)
    ...
    def create_stop_order(
        ...
        if triggerPrice is None:
            raise ArgumentsRequired(
                self.id + " create_stop_order() requires a stopPrice argument"
            )
        query = self.extend(params, {"stopPrice": triggerPrice})
        return self.create_order(symbol, type, side, amount, price, query)

    def create_stop_order_ws(
        self,
        symbol: str,
        type: OrderType,
        side: OrderSide,
        ...
        if triggerPrice is None:
            raise ArgumentsRequired(
                self.id + " createStopOrderWs() requires a stopPrice argument"
            )
        query = self.extend(params, {"stopPrice": triggerPrice})
        return self.create_order_ws(symbol, type, side, amount, price, query)
    ...
    def create_stop_limit_order_ws(
        ...
        if not self.has["createStopLimitOrderWs"]:
            raise NotSupported(
                self.id + " createStopLimitOrderWs() is not supported yet"
            )
        query = self.extend(params, {"stopPrice": triggerPrice})
    ...
    def create_stop_market_order(
        ...
        if not self.has["createStopMarketOrder"]:
            raise NotSupported(
                self.id + " createStopMarketOrder() is not supported yet"
            )
        query = self.extend(params, {"stopPrice": triggerPrice})
    ...
    def create_stop_market_order_ws(
        ...
        if not self.has["createStopMarketOrderWs"]:
            raise NotSupported(
                self.id + " createStopMarketOrderWs() is not supported yet"
            )
        query = self.extend(params, {"stopPrice": triggerPrice})
    ...
    def filter_by_currency_since_limit(
        self, array, code=None, since: Int = None, limit: Int = None, tail=False
    ):
        return self.filter_by_value_since_limit(
    ...
    def parse_last_prices(self, pricesData, symbols: List[str] = None, params={}):
        ...
        if isinstance(pricesData, list):
            ...
            for i in range(0, len(marketIds)):
                ...
                priceData = self.extend(
                    self.parse_last_price(pricesData[marketId], market), params
                )
                results.append(priceData)
        symbols = self.market_symbols(symbols)
    ...
    def parse_deposit_addresses(
        self, addresses, codes: Strings = None, indexed=True, params={}
    ):
        result = []
    ...
    def parse_funding_rate_histories(
        self, response, market=None, since: Int = None, limit: Int = None
    ):
        rates = []
        for i in range(0, len(response)):
            entry = response[i]
    ...
    def parse_long_short_ratio_history(
        self, response, market=None, since: Int = None, limit: Int = None
    ):
        rates = []
        for i in range(0, len(response)):
    ...
    def handle_trigger_direction_and_params(
        self, params, exchangeSpecificKey: Str = None, allowEmpty: Bool = False
    ):
        """
        @ignore
        ...
        if (
            not self.in_array(
                triggerDirection,
                ["ascending", "descending", "up", "down", "above", "below"],
            )
    ...
    def parse_open_interests_history(
        self, response, market=None, since: Int = None, limit: Int = None
    ):
        interests = []
        for i in range(0, len(response)):
    ...
    def fetch_funding_rate(self, symbol: str, params={}):
        if self.has["fetchFundingRates"]:
            ...
            if not market["contract"]:
                raise BadSymbol(
                    self.id + " fetchFundingRate() supports contract markets only"
                )
            rates = self.fetch_funding_rates([symbol], params)
            rate = self.safe_value(rates, symbol)
            if rate is None:
                raise NullResponse(
                    self.id + " fetchFundingRate() returned no data for " + symbol
                )
            else:
                return rate
        else:
    ...
    def fetch_funding_interval(self, symbol: str, params={}):
        if self.has["fetchFundingIntervals"]:
            ...
            if not market["contract"]:
                raise BadSymbol(
                    self.id + " fetchFundingInterval() supports contract markets only"
                )
            rates = self.fetch_funding_intervals([symbol], params)
            rate = self.safe_value(rates, symbol)
            if rate is None:
                raise NullResponse(
                    self.id + " fetchFundingInterval() returned no data for " + symbol
                )
            else:
                return rate
        else:
    ...
    def fetch_mark_ohlcv(
        ...
        if self.has["fetchMarkOHLCV"]:
            ...
            return self.fetch_ohlcv(
                symbol, timeframe, since, limit, self.extend(request, params)
            )
        else:
    ...
    def fetch_premium_index_ohlcv(
        ...
        if self.has["fetchPremiumIndexOHLCV"]:
            ...
            raise NotSupported(
                self.id + " fetchPremiumIndexOHLCV() is not supported yet"
            )
    ...
    def handle_time_in_force(self, params={}):
        ...
        """
        timeInForce = self.safe_string_upper(
            params, "timeInForce"
        ...
        if timeInForce is not None:
            ...
            if exchangeValue is None:
                raise ExchangeError(
                    self.id + ' does not support timeInForce "' + timeInForce + '"'
                )
    ...
    def check_required_argument(
        self, methodName: str, argument, argumentName, options=[]
    ):
        """
        @ignore
        ...
        if (argument is None) or (
            (optionsLength > 0) and (not (self.in_array(argument, options)))
        ):
    ...
    def check_required_margin_argument(
        self, methodName: str, symbol: Str, marginMode: str
    ):
        """
        @ignore
    ...
    def parse_deposit_withdraw_fees(
        self, response, codes: Strings = None, currencyIdKey=None
    ):
        """
        @ignore
        ...
        for i in range(0, len(responseKeys)):
            ...
            currencyId = (
                self.safe_string(dictionary, currencyIdKey) if isArray else entry
            )
            currency = self.safe_currency(currencyId)
            code = self.safe_string(currency, "code")
            if (codes is None) or (self.in_array(code, codes)):
                depositWithdrawFees[code] = self.parse_deposit_withdraw_fee(
                    dictionary, currency
                )
    ...
    def filter_by_array_positions(
        self, objects, key: IndexType, values=None, indexed=True
    ):
        """
        @ignore
    ...
    def handle_max_entries_per_request_and_params(
        self, method: str, maxEntriesPerRequest: Int = None, params={}
    ):
        newMaxEntriesPerRequest = None
        newMaxEntriesPerRequest, params = self.handle_option_and_params(
            params, method, "maxEntriesPerRequest"
        )
        if (newMaxEntriesPerRequest is not None) and (
            newMaxEntriesPerRequest != maxEntriesPerRequest
        ):
    ...
    def fetch_paginated_call_dynamic(
        ...
        maxCalls = None
        maxCalls, params = self.handle_option_and_params(
            params, method, "paginationCalls", 10
        )
        maxRetries = None
        maxRetries, params = self.handle_option_and_params(
            params, method, "maxRetries", 3
        )
        paginationDirection = None
        paginationDirection, params = self.handle_option_and_params(
            params, method, "paginationDirection", "backward"
        )
        paginationTimestamp = None
        removeRepeatedOption = removeRepeated
        removeRepeatedOption, params = self.handle_option_and_params(
            params, method, "removeRepeated", removeRepeated
        )
        calls = 0
        result = []
        errors = 0
        until = self.safe_integer_n(
            params, ["until", "untill", "till"]
            ...
            try:
                if paginationDirection == "backward":
                    ...
                    response = getattr(self, method)(
                        symbol, None, maxEntriesPerRequest, params
                    )
                    responseLength = len(response)
                    if self.verbose:
                        backwardMessage = (
                            "Dynamic pagination call "
                            + self.number_to_string(calls)
                            + " method "
                            + method
                            + " response length "
                    ...
                    paginationTimestamp = self.safe_integer_2(
                        firstElement, "timestamp", 0
                    )
                    ...
                    response = getattr(self, method)(
                        symbol, paginationTimestamp, maxEntriesPerRequest, params
                    )
                    responseLength = len(response)
                    if self.verbose:
                        forwardMessage = (
    ...
    def fetch_paginated_call_deterministic(
        ...
        if since is not None:
            ...
            currentSince = max(
                currentSince, 1241440531000
            )  # avoid timestamps older than 2009
    ...
    def fetch_paginated_call_cursor(
        self,
        method: str,
        symbol: Str = None,
        since=None,
        ...
        while i < maxCalls:
            try:
                ...
                if method == "fetchAccounts":
                    ...
                    method == "getLeverageTiersPaginated" or method == "fetchPositions"
                ):
                    response = getattr(self, method)(symbol, params)
                elif method == "fetchOpenInterestHistory":
                    response = getattr(self, method)(
                        symbol, timeframe, since, maxEntriesPerRequest, params
                    )
                else:
                    response = getattr(self, method)(
                        symbol, since, maxEntriesPerRequest, params
                    )
                errors = 0
                responseLength = len(response)
                if self.verbose:
    ...
    def fetch_paginated_call_incremental(
        self,
        method: str,
        symbol: Str = None,
        since=None,
    ...
    def remove_repeated_elements_from_array(
        ...
        for i in range(0, len(input)):
            ...
            uniqValue = (
                ...
                else self.safe_string(entry, "id")
            )
    ...
    def parse_option(
        self, chain: dict, currency: Currency = None, market: Market = None
    ):
    ...
    def parse_option_chain(
        ...
        for i in range(0, len(response)):
            ...
            optionStructures[market["symbol"]] = self.parse_option(
                info, currency, market
            )
        return optionStructures

    def parse_margin_modes(
        self,
        ...
        for i in range(0, len(response)):
            ...
            if (symbols is None) or self.in_array(market["symbol"], symbols):
                marginModeStructures[market["symbol"]] = self.parse_margin_mode(
                    info, market
                )
    ...
    def parse_conversions(
        ...
        for i in range(0, len(conversions)):
            ...
            conversion = self.extend(
                self.parse_conversion(entry, fromCurrency, toCurrency), params
            )
    ...
    def parse_margin_modifications(
        self,
        response: List[object],
        symbols: Strings = None,
        symbolKey: Str = None,
    ...
    def clean_cache(self, subscription: dict):
        ...
        symbolsLength = len(symbols)
        if topic == "ohlcv":
            symbolsAndTimeframes = self.safe_list(
                subscription, "symbolsAndTimeframes", []
            )
