...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        # Use explicit None checks for dict-like properties for slightly faster dispatch
        ...
        for name in dir(self):
            if name and name[0] != "_" and name[-1] != "_" and "_" in name:
                parts = name.split("_")
    ...
    def safe_string(dictionary, key, default_value=None):
        # Inline key existence test for dict, avoids heavy hasattr/__getitem__
        # and optimizes for dicts -- most common case
        try:
            val = dictionary[key]
    ...
    def safe_integer(dictionary, key, default_value=None):
        # Inline key test and avoid double conversion
        try:
            val = dictionary[key]
        except (KeyError, IndexError, TypeError):
            return default_value
    ...
    def iso8601(timestamp=None):
        # Fast-path: verify input quickly
        ...
        try:
            ...
            formatted = dt.strftime("%Y-%m-%dT%H:%M:%S.") + f"{ms:03d}Z"
            return formatted
    ...
    def safe_order(self, order: dict, market: Market = None):
        ...
        if timeInForce is None:
            if not isTriggerOrSLTpOrder and (
                self.safe_string(order, "type") == "market"
            ):
                timeInForce = "IOC"
        ...
        triggerPrice = self.parse_number(
            self.safe_string_2(order, "triggerPrice", "stopPrice")
        )
    ...
    def safe_ticker(self, ticker: dict, market: Market = None):
        # Cache local bindings for hot functions
        ...
        safe_dict = getattr(
            self, "safe_dict", None
        )  # may not exist; fallback if needed

        open = omit_zero(safe_string(ticker, "open"))
        close = omit_zero(safe_string_2(ticker, "close", "last"))
        change = omit_zero(safe_string(ticker, "change"))
        percentage = omit_zero(safe_string(ticker, "percentage"))
        average = omit_zero(safe_string(ticker, "average"))
        vwap = safe_string(ticker, "vwap")
        baseVolume = safe_string(ticker, "baseVolume")
        quoteVolume = safe_string(ticker, "quoteVolume")
        if vwap is None:
            vwap = Precise.string_div(omit_zero(quoteVolume), baseVolume)
        # Calculate open
        # calculate open
        ...
        if open is not None:
            ...
            if average is None and close is not None:
                ...
                if (
                    market is not None
                    and getattr(self, "is_tick_precision", lambda: False)()
                ):
                    marketPrecision = None
                    if safe_dict:
                        marketPrecision = safe_dict(market, "precision")
                    else:
                        marketPrecision = market.get("precision", None)
                    precisionPrice = (
                        safe_string(marketPrecision, "price")
                        if marketPrecision
                        else None
                    )
        ...
        closeParsed = parse_number(omit_zero(close))
        # Batch extended-dict construction, use locals if available
        return self.extend(
            ticker,
            {
                "bid": parse_number(omit_zero(safe_string(ticker, "bid"))),
                "bidVolume": safe_number(ticker, "bidVolume"),
                "ask": parse_number(omit_zero(safe_string(ticker, "ask"))),
                "askVolume": safe_number(ticker, "askVolume"),
                "high": parse_number(omit_zero(safe_string(ticker, "high"))),
                "low": parse_number(omit_zero(safe_string(ticker, "low"))),
                "open": parse_number(omit_zero(open)),
                "close": closeParsed,
                "last": closeParsed,
                "change": parse_number(change),
                "percentage": parse_number(percentage),
                "average": parse_number(average),
                "vwap": parse_number(vwap),
                "baseVolume": parse_number(baseVolume),
                "quoteVolume": parse_number(quoteVolume),
                "previousClose": safe_number(ticker, "previousClose"),
                "indexPrice": safe_number(ticker, "indexPrice"),
                "markPrice": safe_number(ticker, "markPrice"),
            },
        )
