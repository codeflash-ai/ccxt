...
class Exchange(object):
    ...
    def safe_value(dictionary, key, default_value=None):
        # Fast path for common dict/str usage, avoiding the extra key_exists call
        if isinstance(dictionary, dict) and isinstance(key, str):
            # this shortcut covers the vast majority of calls
            return dictionary.get(key, default_value)
        # fallback to legacy, full method when needed (e.g. list/other mapping types)
        return (
    ...
    @staticmethod
    def extend(*args):
        # optimized: no repeated type checks, preallocate once, uses fast dict.update
        if not args:
            return {}
        first_arg = args[0]
        use_ordered = type(first_arg) is collections.OrderedDict
        result = collections.OrderedDict() if use_ordered else {}
        for arg in args:
            if arg:
                result.update(arg)
        return result
    ...
    def safe_list(
        ...
        @ignore
               ...
               :returns Array | None:
        """
        # Fast path: if dictionaryOrList is a dict and key is present with list value
        if isinstance(dictionaryOrList, dict) and isinstance(key, str):  # hot path
            v = dictionaryOrList.get(key, defaultValue)
            if v is None:
                return defaultValue
            if isinstance(v, list):
                return v
            return defaultValue
        # fallback to slower safe_list_n for non-dict/str keys or unusual cases
